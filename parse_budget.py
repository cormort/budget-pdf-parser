...
# 句中「科目名＋N千元」配對，用於拆解內嵌於標題句的明細
INLINE_PAIR_REGEX = re.compile(r"([一-鿿（）()]{2,22}?)([\d,]+)\s*千\s*元")


def _strip_lead(s: str) -> str:
    """只從字首剝除贅詞/標點。不可用 ^.*?(…|及|、)，否則會砍到科目名內部的
    「及」「、」（如 農業與園藝用品「及」環境美化費、捐助「、」補助與獎助）。"""
    prev = None
    while prev != s:
        prev = s
        s = re.sub(r"^[，,、及和與：:]+", "", s)
        s = re.sub(r"^(包括|係|編列)", "", s)
    return s


def inline_pairs(text: str):
    """從標題句抽出可比對到科目表的「科目名＋金額」配對。"""
    out = []
    for mm in INLINE_PAIR_REGEX.finditer(text):
        name, amt = mm.group(1), mm.group(2)
        m = match_account(_strip_lead(name))
        if m and len(m["code"]) >= 2:
            out.append({
                "code": m["code"],
                "amount": int(amt.replace(",", "")) * 1000,
                "frag": mm.group(0),
            })
    return out
...
def parse(text: str, fund: str = ""):
    ...
    for raw in text.split("\n"):
        ...
        # 內嵌明細拆列：標題句把二/三級科目與金額寫在同一句（本列 l2 為空）。
        # 安全閥：僅當「子項加總 == 母列金額」才拆，對不上就維持原狀。
        if row_l1 and not row_l2 and amount:
            pairs = inline_pairs(line)
            if len(pairs) >= 2 and sum(p["amount"] for p in pairs) == amount:
                base_row["is_subtotal"] = True
                for p in pairs:
                    code = p["code"]
                    rows.append(
                        {
                            **base_row,
                            "l1": CODE_TO_NAME.get(code[:1], row_l1),
                            "l2": CODE_TO_NAME.get(code[:2], ""),
                            "l3": CODE_TO_NAME.get(code, "") if len(code) >= 3 else "",
                            "amount": p["amount"],
                            "description": p["frag"],   # 子列描述＝該科目片段，勿繼承母列(含「包括」會被小計規則濾掉)
                            "raw": p["frag"],
                            "is_subtotal": False,
                            "derived": True,
                        }
